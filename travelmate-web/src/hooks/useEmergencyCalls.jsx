...
export function useEmergencyCalls() {
  ...
  const createEmergencyCall = useCallback(async ({ travelGroupId, latitude, longitude, emergencyType, comment }) => {
    try {
      const response = await axiosInstance.post('/emergency-calls', {
        travelGroupId,
        latitude,
        longitude,
        emergencyType,
  ...
  const getActiveEmergencyCall = useCallback(async (travelGroupId) => {
    try {
      const response = await axiosInstance.get(`/emergency-calls/active/${travelGroupId}`);
      return response.data;
    } catch (error) {
  ...
  const resolveEmergencyCall = useCallback(async (callId) => {
    try {
      const response = await axiosInstance.post(`/emergency-calls/${callId}/resolve`);
      return response.data;
    } catch (error) {
